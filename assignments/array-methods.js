...
// Problem 1
// The fun run was successful, but the director has agoraphobia and wants next year's invite to have fewer people. You tried explaining what a fundraiser is and it didn't change their mind. The director tells you to create a new array of runners whose companies donated at least $150.
let largeDonors = runners.filter(amount => {
  return amount.donation >= 150;
});
...
// Problem 3
// The drunk director has decided that they want to found a tech startup with a name 'inspired' by one of the donor names. Since they want their startup to be cool, the director wants some vowels removed from each donor name. They have insisted on having a list of these "tech ready" names ordered alphabetically so they can print it out for their vision board. Create a coolNames array to store the new company names and log the result. (You should probably quit your job while you're ahead, your boss is no good)
let coolNames = runners.map(company => {
  return company.company_name.replace(/[eu]/gi, "");
});
coolNames.sort();
